    ...
    <Container className="pt-12 pb-10 text-center sm:pt-16 sm:pb-12 md:pt-20 md:pb-16 lg:pt-32">
      <h1 className="mx-auto max-w-4xl font-display text-4xl font-medium tracking-tight text-slate-900 sm:text-5xl md:text-6xl lg:text-7xl">
        ...
        <span className="relative whitespace-nowrap text-blue-600">
          ...
          {/* <span className="relative">instantly delivered</span> */}
        </span>
      </h1>
      ...
      <div className="mt-8 flex flex-col items-center gap-4 sm:mt-10 sm:flex-row sm:justify-center sm:gap-x-6">
        ...
        >
          <svg
            aria-hidden="true"
            className="h-4 w-4 flex-none fill-blue-600 group-active:fill-current sm:h-5 sm:w-5"
          >
      ...
      {/* <div className="mt-24 sm:mt-28 md:mt-32 lg:mt-44">
        <p className="font-display text-base text-slate-900 sm:text-lg">
          Currently Operating In
        </p>
        <ul
          role="list"
          className="mt-8 flex flex-wrap items-center justify-center gap-x-12 gap-y-8 sm:mt-10 sm:gap-x-16 md:gap-x-20"
        >
          {[
            [
              { name: 'Delhi', logo: logoTransistor },
              { name: 'Noida', logo: logoTuple },
              { name: 'Gurugram', logo: logoStaticKit },
            ],
            [
              { name: 'Bikaner', logo: logoMirage },
              { name: 'Faridabad', logo: logoLaravel },
            ],
          ].map((group, groupIndex) => (
            <li
              key={groupIndex}
              className="flex flex-wrap justify-center gap-x-12 gap-y-8 sm:gap-x-16 md:gap-x-20"
            >
              {group.map((company) => (
                <li key={company.name} className="flex">
                  <span className="font-display text-xl font-medium text-slate-900 sm:text-2xl">
                    {company.name}
                  </span>
                </li>
              ))}
